...
> serde_json::Value gemini_ask_gemini_a_question(prompt, country, web_search, image_url)
...
**image_url** | Option<**String**> | UNAVAILABLE. Signed-out Gemini refuses image questions — it accepts the upload and then answers PERMISSION_DENIED to its own generate RPC, so this returns 422 immediately rather than charging for a render that cannot succeed. Use /v1/chatgpt/ask for image input. |  |
